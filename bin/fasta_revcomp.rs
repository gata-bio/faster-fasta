...
//! fasta-revcomp reads.fastq --output revcomp.fastq
//! cat sequences.fasta | fasta-revcomp | fasta-sample --fraction 0.1
//! ```
//!
//! Exit: 0 complemented a record, 1 ran and complemented none, 2 could not run.
...
fn run(args: &Args) -> io::Result<RunOutcome> {
    ...
    if args.dry_run {
        eprintln!("would complement {converted} records; nothing was written");
